import type { RenderableTreeNode, RenderableTreeNodes, Scalar } from "@markdoc/markdoc";
...
const render = (node: RenderableTreeNodes): string => {
  ...
  const getOutput = () => {
    ...
    if (children?.length) {
      const escapeChildrenToString = (arrayOfChildren: string | {
        [key: string]: Scalar;
    } | RenderableTreeNode[]): string => {
        if (typeof arrayOfChildren === "string") return escapeCharacters(arrayOfChildren)
        if (Array.isArray(arrayOfChildren)){
          return arrayOfChildren.map(child => 
            escapeCharacters(
              child?.
                toString().
                replace("\n","") // Don't break the string in the middle
                || '' // Make sure we end up with strings, not null or undefined
